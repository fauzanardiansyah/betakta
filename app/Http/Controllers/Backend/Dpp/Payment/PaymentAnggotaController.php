...
class PaymentAnggotaController extends Controller
{
    ...
    public function getDataPaymentAnggota()
    {
        ...
        ->select('t_payment_confirmation.no_invoice' , 't_payment_confirmation.atas_nama','t_payment_confirmation.nama_bank_anda','t_payment_confirmation.nominal','t_payment_confirmation.created_at','t_payment_confirmation.id','t_registrasi_users.nm_bu', 't_invoice_kta.status_pembayaran', 't_detail_kta.id as id_detail_kta', 't_app_kta.status_pengajuan')
        ->where('t_dp.id', Session::get('id_dp'))
        // ->groupBy('t_payment_confirmation.no_invoice' , 't_payment_confirmation.atas_nama','t_payment_confirmation.nama_bank_anda','t_payment_confirmation.nominal','t_payment_confirmation.created_at','t_payment_confirmation.id','t_registrasi_users.nm_bu', 't_invoice_kta.status_pembayaran', 't_detail_kta.id as id_detail_kta', 't_app_kta.status_pengajuan')
        // ->groupBy('t_payment_confirmation.no_invoice' , 't_payment_confirmation.atas_nama','t_payment_confirmation.nama_bank_anda','t_payment_confirmation.nominal','t_payment_confirmation.created_at','t_payment_confirmation.id')
        ->orderBy('t_payment_confirmation.created_at','desc')
        ;

        // dd($dataPaymentAnggota->get());
        
        
        
        return Datatables::of($dataPaymentAnggota)
        ->addColumn('action', function ($detailPaymnet) {
            if ($detailPaymnet->status_pengajuan == 7 || $detailPaymnet->status_pengajuan == 3 || $detailPaymnet->status_pengajuan == 6|| $detailPaymnet->status_pengajuan == 8) {
               return '
            ...
            else
            {
                 return '
                <a href="#" class="btn btn-sm btn-default"  id="show-detail-payment" data-id-detail-kta="'.$detailPaymnet->id_detail_kta.'" data-id-payment="'.$detailPaymnet->id.'"  title="Show detail payment"><i class="fa fa-search"></i></a>
                <a href="'.($detailPaymnet->status_pembayaran != 0 || $detailPaymnet->status_pengajuan == 9  ? ''.route('dpp.payment.verify', ['id_detail_kta' => $detailPaymnet->id_detail_kta]).'' : "#").'     " class="btn btn-sm btn-success"   data-id-payment="'.$detailPaymnet->id_detail_kta.'"  title="DPP Verified" '.($detailPaymnet->status_pembayaran != 0 || $detailPaymnet->status_pengajuan == 9 ? "" : "disabled").'><i class="fa fa-check"></i></a>
                ';
            }
    ...
    public function getDetailDataPaymentAnggota(Request $request)
    {
        ...
        $dataPaymentAnggota = PaymentConfirmation::with('invoice')->findOrfail($idPayment);
    ...
    public function acceptPaymentAnggota(Request $request)
    {
        ...
        if ($updataStatusPembayaran) {
            ...
            ->update([
                'status_pengajuan'=>9,
                'keterangan' => 'Pembayaran anda telah terverifikasi oleh team KTA inkindo, Silahkan menunggu team KTA DPP inkindo menyelesaikan pengajuan anda',
            ]);
    ...
    public function dppVerifiyAnggota($id_detail_kta)
    {
        ...
        $paymentConfirmation = \App\InvoicePengajuanKta::where('id_detail_kta',$id_detail_kta)->update(['status_pembayaran' => 1]);
