...
class PaymentDpnController extends Controller
{
    ...
    public function acceptPaymentRoleShare(Request $request)
    {
        ...
        try {
            ...
            foreach ($request->dataDetail as $key => $value) {
                 HistoryApprovalPengajuan::where('id_detail_kta', $value)
                ->update(
                    [
                        'status_pengajuan' => 6,
            ...
            return response()->json(['errors' => 'Gagal memverifikasi pembayaran role sharing'], 422);
            DB::rollBack();
        }
    }
