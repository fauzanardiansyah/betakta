...
class DownloadKtaController extends Controller
{
    ...
    public function index()
    {
        ...
        // ->orWhere('t_app_kta.status_pengajuan', 6)
        // ->orWhere('t_app_kta.status_pengajuan', 5)
       ...
       return view('backend/anggota/content-pages/download-kta.main-download', compact('memberData')); 
    }
    ...
    public function download_kta($idKta)
    {
        $dataKta = DB::table('t_kta')
                     ->join('t_dp', 't_kta.id_dp', '=', 't_dp.id')
                     ->join('provinsi', 't_dp.id_provinsi', '=', 'provinsi.id')
        ...
        // return view('prints-template.kta_download', compact('dataKta','dpnSignature'));

        if ($dataKta) {
            $pdf = PDF::loadView('prints-template.kta_download', compact('dataKta','dpnSignature'))->setPaper('a4', 'landscape');
            return $pdf->stream('invoice |'.$dataKta->nm_bu.'.pdf');
        } else {
            abort(404);
        }
