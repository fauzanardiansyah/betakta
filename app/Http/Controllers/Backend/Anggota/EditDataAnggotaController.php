...
class EditDataAnggotaController extends Controller
{
    public function formAdminstrasiBadanUsaha()
    {
        ...
        return view('backend/anggota/content-pages/edit_data.form_badan_usaha', compact('dataUser'));
    }
    ...
    public function formPenanggungJawabBadanUsaha()
    {
        $dataUser = \DB::table('t_registrasi_users as tru')
            ...
            ->whereNotNull('tk.no_kta')
            ->first();
        return view('backend/anggota/content-pages/edit_data.form_penanggung_jawab',compact('dataUser'));
    }
    ...
    public function formLegalitasBadanUsaha()
    {
        ...
        return view('backend/anggota/content-pages/edit_data.form_legalitas',compact('dataUser'));
    }

    public function formDokumenPendukung()
    {
        return view('backend/anggota/content-pages/edit_data.form_pendukung');
    }
    ...
    public function infoUmumBadanUsaha(Request $request)
    {
        ...
        if($request->input('provinsi') == 17) {
            return redirect()->back()->with('preventDKI', 'Sedang dalam Proses Integrasi DPN & DPP DKI');
        }
        ...
        if (!$validator->fails()) {
            $dataInfoBu = [
                ...
                'status_kta'          => 1,
                'status_penataran'    => 1,
                'edit_data_until_date' => date('Y-m-d H:i:s', strtotime('+11 month'))
            ];
            ...
            if (\App\Kta::create($dataInfoBu)) {
                ...
                if ($detailKta->save()) {
                    ...
                    return redirect()->route('anggota.edit_data.formAdministrasiBadanUsaha');
                }
            }
    ...
    public function administrasiBadanUsaha(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            $update = \App\DataAdministrasiBadanUsaha::where('id_detail_kta',$get_data->id)
            ->update($dataAdministrasiBu);
            

            return redirect()->route('anggota.edit_data.formPenanggungJawabBadanUsaha');
    ...
    public function penanggungJawabBadanUsaha(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            // \App\DetailKta::whereId($get_data->id) ->update(['is_inserted' => 2]);
            return redirect()->route('anggota.edit_data.formLegalitasBadanUsaha');
    ...
    public function legalitasBadanUsaha(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            if ($request->has('no_akte_perubahan') || $request->has('no_sk_perubahan')) {
                ...
                $update = \App\DataLegalitasBadanUsaha::where('id_detail_kta',$get_data->id)
                ->update($legalitasBu);
                ...
                if ($update) {
                    ...
                    return redirect()->route('anggota.edit_data.formUploadDokumenPendukung');
                } else {
                    return redirect()->route('anggota.edit_data.formLegalitasBadanUsaha');
                }
            } else {
        ...
        if (!$validator->fails()) 
        {
            ...
            // dd('tes');
             ...
             \App\DataDokumenPendukungBadanUsaha::where('id_detail_kta',$get_data->id)->update([
                ...
                \App\DetailKta::whereId($get_data->id)->update(['is_inserted' => 4,'jenis_pengajuan'=>8]);
                \App\HistoryApprovalPengajuan::where('id_detail_kta',$get_data->id)->update(['status_pengajuan'=>0, 'keterangan'=>'Dokumen anda telah memasuki fase "Screening" oleh Team KTA Online Inkindo, Jika dokumen anda lengkap dan valid maka akan di lanjutkan ke fase berikutnya.']);
