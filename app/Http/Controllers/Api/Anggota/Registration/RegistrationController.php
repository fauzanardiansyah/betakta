...
namespace App\Http\Controllers\Api\Anggota\Registration;
...
class RegistrationController extends Controller
{
    ...
    public function getCouncil()
    {
        ...
        if (! empty($councils)) {
            $this->data = [
                'data' => $councils,
                'message' => 'Data pegurus di temukan',
                'status' => 200
            ];
            ...
            $this->data = [
                'data' => null,
                'message' => 'Data pegurus tidak di temukan',
                'status' => 404
            ];
    ...
    public function infoUmumBadanUsaha(Request $request)
    {
        ...
        if (!empty($checkLastKta) && $checkLastKta->status_kta == 0) {
            return response()->json([
                'message' => 'Nomor KTA anda masih aktif',
                'status' => 400
            ], 400);
        }
            ...
            if (\App\Kta::create($dataInfoBu)) {
                ...
                if ($detailKta->save()) {
                    ...
                    $this->data = [
                        ...
                        'status' => 200,
                        'redirect' => [
                            'url' => route('api.registration.administration-bu'),
                            'method' => 'POST'
                        ]
                    ];
    ...
    public function administrasiBadanUsaha(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            if (\App\DataAdministrasiBadanUsaha::create($dataAdministrasiBu)) {
                $detailKta = \App\DetailKta::whereId($request->id_detail_kta) ->update(['is_inserted' => 1]);
                $this->data = [
                    'data' => [
                        'id_detail_kta' => $request->id_detail_kta
                    ],
                    ...
                    'status' => 200,
                    'redirect' => [
                        'url' => route('api.registration.pjbu'),
                        'method' => 'POST'
                    ]
                ];
    ...
    public function penanggungJawabBadanUsaha(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'email_pjbu'              => 'required|email|string|max:30',
        ...
        if (!$validator->fails()) {
            ...
            if (\App\DataPenanggungJawabBadanUsaha::create($dataPjbu)) {
                ...
                $this->data = [
                    ...
                    'status' => 200,
                    'redirect' => [
                        'url' => route('api.registration.legality'),
                        'method' => 'POST'
                    ]
                ];
