...
<div class="app-main__inner">
    ...
    <div class="tab-pane tabs-animation fade active show" id="tab-content-1" role="tabpanel">
        <div class="main-card mb-3 card">
            <div class="card-body">
                <h5 class="card-title">Form Edit Akun Dewan Pengurus Provinsi</h5>
                <form action="{{ route('administrator.dewan.update-akun-dpp-process', ['id_users_dp' => $data_users_dpp->id ]) }}" method="POST">
                    @csrf
                    ...
                    <div class="col-sm-10">
                        <select name="id_dp" class="form-control" id="">
                            @foreach ($data_dpp as $provinsi_item)
                                <option value="{{ $provinsi_item->id }}" {{ ($provinsi_item->id == $data_users_dpp->id) ?  'selected' : '' }}>{{ $provinsi_item->provinsi->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="position-relative row form-group"><label for="exampleEmail"
                        ...
                        <div class="col-sm-10"><input name="npwp_pengurus" id="exampleEmail" placeholder="NPWP Pengurus"
                                type="text" class="form-control" value="{{ $data_users_dpp->npwp_pengurus }}" required>
                            {!! $errors->first('npwp_pengurus', '<p class=""
                                style="margin-top:3px;padding:5px;background:#CE5454; color:cornsilk">:message</p>')
                            !!}
                        </div>
                    ...
                    <div class="position-relative row form-group"><label for="examplePassword"
                        ...
                        <div class="col-sm-10"><input name="email_pengurus" id="examplePassword" value="{{ $data_users_dpp->email_pengurus }}" type="text" class="form-control"  placeholder="Email Pengurus" type="text" class="form-control"  required>
