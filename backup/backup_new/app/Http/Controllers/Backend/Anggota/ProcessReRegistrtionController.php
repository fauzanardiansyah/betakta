...
class ProcessReRegistrtionController extends Controller
{
    public function infoUmumBadanUsaha(Request $request)
    {
        ...
        if($cekJenisPengajuan > 0) {
            return redirect()->route('anggota.registration.infoUmumBadanUsaha')->with('preventReJenisPengajuan', 'Pengajuan Baru Anda sudah ada dan sedang di proses');
        }
       
                                
        if($request->input('provinsi') == 17) {
            return redirect()->route('anggota.registration.infoUmumBadanUsaha')->with('preventDKI', 'Sedang dalam Proses Integrasi DPN & DPP DKI');
        }
        if (!empty($checkLastKta) && $checkLastKta->status_kta == 0) {
            return redirect()->route('anggota.registration.infoUmumBadanUsaha')->with('ktaStillActive', 'Nomor KTA anda masih aktif');
        } 
        ...
        if (!$validator->fails()) {
            ...
            return redirect()->route('anggota.registration.infoUmumBadanUsaha');
        }
    ...
    public function administrasiBadanUsaha(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            if (\App\DataAdministrasiBadanUsaha::create($dataAdministrasiBu)) {
                ...
                return redirect()->route('anggota.re-registration.formPenanggungJawabBadanUsaha');
            } else {
                return redirect()->route('anggota.registration.administrasiBadanUsaha');
            }
        } else {
            return redirect()->route('anggota.registration.administrasiBadanUsaha');
        }
    ...
    public function penanggungJawabBadanUsaha(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            if (\App\DataPenanggungJawabBadanUsaha::create($dataPjbu)) {
                ...
                return redirect()->route('anggota.re-registration.formLegalitasBadanUsaha');
            } else {
                return redirect()->route('anggota.registration.penanggungJawabBadanUsaha');
            }
        } else {
            return redirect()->route('anggota.registration.penanggungJawabBadanUsaha')
        ...
        if (!$validator->fails()) {
            if ($request->has('no_akte_perubahan') || $request->has('no_sk_perubahan')) {
                ...
                if ($idLastLegalitas = DB::table('t_legalitas_kta')->insertGetId($legalitasBu)) {
                    ...
                    return redirect()->route('anggota.re-registration.formUploadDokumenPendukung');
                } else {
                    return redirect()->route('anggota.re-registration.formLegalitasBadanUsaha');
                }
            } else {
            ...
            return redirect()->route('anggota.registration.legalitasBadanUsaha')
            ->withErrors($validator);
        }
    }
    ...
    public function dokumenPendukung(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            if ($file) {
                ...
                return redirect()->route('anggota.registration.dokumen');
