...
class InvoiceDpnController extends Controller
{
    ...
    public function getAnggotaLokal()
    {
        ...
        ->addColumn('action', function ($kta) {
            $countTerbitRoleShare = count(InvoiceRoleShare::where('id_detail_kta', $kta->id_detail_kta)->get()); 
            if ($kta->status_pengajuan == 7) :
                ...
                elseif ($kta->status_pengajuan == 0 || $kta->status_pengajuan == 1 || $kta->status_pengajuan == 4) :
    ...
    public function publishInvoiceRoleShare(Request $request)
    {
        ...
        if ($publishInvoiceRoleShare) {
                ...
                dispatch(new SendInvoiceRoleShareMail($dataInvoiceRoleShare));
    ...
    public function showInvoice($noInvoice, $idDetailKta)
    {
        ...
        $pengajuan  = DB::table('t_kta')
                      ...
                      ->select(
                          ...
                          't_administrasi_kta.no_telp',
                          't_detail_kta.id as id_detail_kta',
                          't_detail_kta.waktu_pengajuan as waktu_pengajuan',
                          't_kta.id_dp',
                          'provinsi.name  as nama_provinsi'
    ...
    public function publishInvoiceAnggotaAfiliasi(Request $request)
    {
        ...
        // $month_afiliasi =  date("m",strtotime($pengajuan->waktu_pengajuan));
        ...
        //     if($month_afiliasi == "01")
        //     {   
        //         $this->jml_tagihan_agt = $pengurus_nasional->iuran_1_thn_besar + $pengurus_nasional->uang_pangkal;         
        //     }
        //     else
        //     {
        //         $v_iuran = \App\Helpers\LocalDate::get_contribution_now($pengurus_nasional->iuran_1_thn_besar,$timeDiff);
        //         if($month_cut < $get_month_now)
        //          {
        //               $price_cut = \App\Helpers\LocalDate::get_contribution_next($pengurus_nasional->iuran_1_thn_besar);
        //          }
     
        //          $jml_tagihan = round($v_iuran) + $price_cut ;
        //          $this->jml_tagihan_agt = $pengurus_nasional->uang_pangkal + $jml_tagihan;
        ...
        $get_invoice_kta = InvoicePengajuanKta::where('id_detail_kta',$idDetailKta)->where('jenis_pengajuan',$pengajuan->jenis_pengajuan)->first();
        ...
        else
        {
            ...
            $dataInvoiceAnggotaAfiliasi = InvoicePengajuanKta::where('id_detail_kta',$idDetailKta)
            ->update([
                'jml_tagihan'       => ($this->jml_tagihan_agt == null) ? 0 : $this->jml_tagihan_agt,
                'tgl_cetak'         => date('Y-m-d H:i:s'),
            ]);
