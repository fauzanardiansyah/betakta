...
class InvoiceRoleSharingController extends Controller
{
    public function index()
    {
        ...
        foreach($invoice_role_sharing as $item) {
            ...
            if($item->jenis_pengajuan == 0)
            {
                $status="Buat Baru";
            }
            elseif($item->jenis_pengajuan == 1)
            {
                $status="Daftar Ulang";
            }
            elseif($item->jenis_pengajuan == 1)
            ...
            $data = [
                ...
                'nm_bu' => $item->nm_bu,
                'jenis_pengajuan' =>$status ,//($item->jenis_pengajuan == 0) ? 'Buat Baru' : (($item->jenis_pengajuan == 1) ? 'Daftar Ulang' : ($item->jenis_pengajuan == 3) ? 'Perpanjangan' : 'Pemberhentian') ,
