...
class MainFrontendController extends Controller
{
    public function index()
    {
        ...
        return view('frontend/base.index', compact('testimonials', 'news', 'sponsorship','warning'));
    }
    ...
    public function detailPengurus()
    {
        $pengurusInkindo = DB::table('t_dp')
        ->join('provinsi', 't_dp.id_provinsi', '=', 'provinsi.id')
        ->select('t_dp.*', 'provinsi.name as nama_provinsi','provinsi.no_urut')
        ->orderBy('provinsi.no_urut','asc')
        ->paginate(6);
