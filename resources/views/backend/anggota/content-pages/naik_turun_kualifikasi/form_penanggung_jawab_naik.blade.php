...
<div class="col-md-12 col-sm-12 col-xs-12">
  <div class="x_panel">
    ...
    <div class="x_content">
      ...
      <form action="{{ route('anggota.naik_turun_kualifikasi.form_legalitas_naik') }}" method="POST"
        ...
        <div class="item form-group">
          ...
          <div class="col-sm-9">
            <input type="text" class="form-control" name="nm_pjbu" id="w4-username" value="{{ ( empty($dataUser->nm_pjbu) ) ? "" : $dataUser->nm_pjbu }}"
        ...
        <div class="item form-group">
          ...
          <div class="col-sm-9">
            <select name="kewarganegaraan" id="kewarganegaraan" class="form-control" required>
              ...
              @if(empty($dataUser->kewarganegaraan))
                <option value="wni" >WNI</option>
                <option value="wna" >WNA</option>
              @else
                <option value="wni" {{ ($dataUser->kewarganegaraan == 'wni') ? "selected" : "" }}>WNI</option>
                <option value="wna" {{ ($dataUser->kewarganegaraan == 'wna') ? "selected" : "" }}>WNA</option>
              @endif
        ...
        <div class="item form-group">
          ...
          <div class="col-sm-9">
            ...
            <input name="nik" id="nik" class="form-control" placeholder="NIK" 
              ...
              type="number" maxlength="17" value="{{ ( empty($dataUser->nik) ) ? "" : $dataUser->nik }}" required />
            @else

            <input name="nik" id="nik" class="form-control" placeholder="NIK" {{($dataUser->kewarganegaraan == 'wna')  ? "disabled" : "" }}
              oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);"
              type="number" maxlength="17" value="{{ ( empty($dataUser->nik) ) ? "" : $dataUser->nik }}" required />
            @endif
        ...
        <div class="item form-group">
          ...
          <div class="col-sm-9">
            @if(empty($dataUser->kewarganegaraan))
            <input type="text" name="passport" id="passport" class="form-control" value="{{ ( empty($dataUser->no_passport) ) ? "" : $dataUser->no_passport }}"
              placeholder="Passport" required />
            @else
            <input type="text" name="passport" id="passport" class="form-control" value="{{ ( empty($dataUser->no_passport) ) ? "" : $dataUser->no_passport }}" {{($dataUser->kewarganegaraan == 'wni')  ? "disabled" : "" }}
              placeholder="Passport" required />
            @endif
          ...
          <div class="col-sm-9">
            <select name="jabatan_pjbu" class="form-control" required>
              ...
              @php 
                // dd('tes');
              @endphp
                <option value="1" {{ ($dataUser->jabatan == '1') ? "selected" : "" }}>Direktur</option>
                <option value="2" {{ ($dataUser->jabatan == '2') ? "selected" : "" }}>Direktur Utama</option> 
                <option value="3" {{ ($dataUser->jabatan == '3') ? "selected" : "" }}>Presiden Direktur</option>        
              @endif
        ...
        <div class="item form-group">
          ...
          <div class="col-sm-9">
            <select name="pendidikan_formal_pjbu" class="form-control" required>
              ...
              @else
                <option value="0" {{ ($dataUser->pendidikan == '0') ? "selected" : "" }}>S1</option>
                <option value="1" {{ ($dataUser->pendidikan == '1') ? "selected" : "" }}>S2</option>
                <option value="2" {{ ($dataUser->pendidikan == '2') ? "selected" : "" }}>S3</option>
                <option value="3" {{ ($dataUser->pendidikan == '3') ? "selected" : "" }}> S3</option>
              @endif
            </select>
        ...
        <div class="item form-group">
          <label class="col-sm-3 control-label" for="w4-username">Tempat, Tanggal Lahir<span
              class="required">*</span></label>
          <div class="col-sm-3">
            <input type="text" class="form-control" name="tempat" value="{{ ( empty($dataUser->tempat) ) ? "" : $dataUser->tempat }}" placeholder="Tempat"
              required>
            {!! $errors->first('tempat', '<p class=""
              style="margin-top:3px;padding:5px;background:#CE5454; color:cornsilk">:message</p>') !!}
          </div>
          <div class="col-sm-3">
            <input type="date" class="form-control" name="tgl_lahir" value="{{ ( empty($dataUser->tgl_lahir) ) ? "" : $dataUser->tgl_lahir }}"
        ...
        <div class="item form-group">
          ...
          <div class="col-sm-9">
            <textarea class="form-control" name="alamat_pjbu" value="" placeholder="Alamat"
              required> {{ ( empty($dataUser->alamat) ) ? "" : $dataUser->alamat }}</textarea>
        ...
        <div class="item form-group">
          ...
          <div class="col-sm-9">
            <input type="text" data-mask="0000-000-000-000" class="form-control" name="no_hp_pjbu"
              value="{{ ( empty($dataUser->no_hp_pjbu) ) ? "" : $dataUser->no_hp_pjbu }}" placeholder="Nomor Handphone" required>
        ...
        <div class="item form-group">
          ...
          <div class="col-sm-9">
            <input type="email" class="form-control" name="email_pjbu" value="{{ ( empty($dataUser->email_pjbu) ) ? "" : $dataUser->email_pjbu }}"
        ...
        <div class="item form-group">
          ...
          <div class="col-sm-9">
            <input type="text" data-mask="00.000.000.0-000.000" max="16" class="form-control npwp" name="no_npwp_pjbu"
              value="{{ ( empty($dataUser->npwp_pjbu) ) ? "" : $dataUser->npwp_pjbu }}" placeholder="NPWP PJBU" required>
