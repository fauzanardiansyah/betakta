    ...
    $('#payment-role-share-datatable').on('click', '#show-detail-payment', function(){
        ...
        $('#detail-roleshare-datatable').DataTable({
            ...
            columns: [
                ...
                { data: 'id_detail_kta', name: 't_app_kta.id_detail_kta','visible':true},
 ...
 $(document).ready(function(){
    $('#accept-payment-role-share').click(function(event){
        ...
        $.each( dataTable, function( key, value ) {
            ...
            dataDetail.push(value.id_detail_kta)   
        });
        $.ajax({
				type: 'POST',
				url: '{!! route('dpn.payment.accept-role-share') !!}',
				data: { _token: csrf_token, dataArray: dataArray,dataDetail:dataDetail, idRoleShareAccumulation : data_id_role_share},
                beforeSend: function(){
